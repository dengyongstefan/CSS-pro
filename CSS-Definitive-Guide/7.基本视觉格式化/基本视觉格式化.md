...
2. 建立行内框：若line-height值为21px，则会在内容区上下加上3px，构成行内框；
3. 建立行框：根据行内框的最高和最低高度建立。

如果出现line-height小于font-size的情况，会使得在内容框上下添加上负值，则时候可能会出现内容溢出行框的情况，由于文本都是按基线对齐的，所以一般是文字内容的上部分会溢出。
